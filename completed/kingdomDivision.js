...
//const bigInt = require('big-integer') //no need because copy pasted it below for hackerrank
  ...
  initializeDifferent() {
    if (this.isLeaf()) {
      ...
      const allWays = this.neighbors.reduce((accum, currentNeighbor) => {
        return multiplyMod1000000007(accum, currentNeighbor.ways);
      }, 1);
      const waysThatStrandThisAlone = this.neighbors.reduce(
        (accum, currentNeighbor) => {
          return multiplyMod1000000007(accum, currentNeighbor.different);
        },
        1
      );
...
function multiplyMod1000000007(in1, in2) {
  const [a, b] = [bigInt(in1), bigInt(in2)];
  return a
    .multiply(b)
    .mod(1000000007)
    .valueOf();
